...
def query_etf_data(logger: Logger, max_etfs: int, ipo_date: datetime) -> pd.DataFrame:
    ...
    etf_data = etf_data.rename(columns={'assetType': 'asset_type', "ipoDate": "ipo_date"})
    # Filter for ETFs on NASDAQ and NYSE
    etf_data = etf_data.loc[(etf_data["asset_type"] == 'ETF') & ((etf_data['exchange'] == 'NASDAQ') | (etf_data['exchange'].str.contains('NYSE')))]
    ...
    logger.info(f"Number of active ETFs on Nasdaq and NYSE as of {datetime.today().strftime('%Y-%m-%d')} with IPO date after {ipo_date.strftime('%Y-%m-%d')}: {etf_data.shape[0]}")

    session = CachedLimiterSession(
        limiter=Limiter(RequestRate(60, Duration.SECOND * 60)),  # Max 60 requests per 1 minute
    ...
    logger.info("Sending GET requests to yahoo finance for each ETF in the list of ETFs from Alpha Vantage")
    yf_data = []
    for ticker in tickers.tickers.values():
        if ticker.info:
            ...
            continue
    logger.info("Completed requesting data from yahoo finance, saving it as a data frame")
    yf_data = pd.DataFrame(yf_data)

    logger.info("Left joining performance data from yahoo finance data onto the etf data using 'symbol' as a key")
    data = pd.merge(
        left=etf_data,
        right=yf_data,
        on='symbol',
