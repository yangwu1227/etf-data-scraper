...
def lambda_handler(event: Dict[str, str], context: Any) -> None:
    ...
    try:
        ...
        version = ecs_client.describe_task_definition(taskDefinition=task_definition)['taskDefinition']['revision'] 
        ...
        logger.error(f"The parameters passed to the `run_task` method are invalid: {error}")
        raise error
